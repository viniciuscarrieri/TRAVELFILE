...
class _TransladoPageState extends State<TransladoPage> {
  ...
  Future<void> listarDocumentos() async {
    final ref = firebase_storage.FirebaseStorage.instance.ref().child('files/${auth.currentUser!.uid}/transfer');
    transladoFiles = (await ref.listAll()).items;
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      floatingActionButton: FloatingActionButton.extended(
        onPressed: () async { await Navigator.of(context).pushNamed('/cad_translado'); setState(() => _futureListar = listarDocumentos()); },
      ...
      body: FutureBuilder<void>(
        ...
        builder: (context, snapshot) {
          ...
          if (transladoFiles.isEmpty) return EmptyStateWidget(icon: Icons.directions_bus, message: 'Nenhum documento de transfer\nencontrado.', actionLabel: 'Adicionar agora', onAction: () => Navigator.of(context).pushNamed('/cad_translado').then((_) => setState(() => _futureListar = listarDocumentos())));
