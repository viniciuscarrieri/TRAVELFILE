...
class _HotelPageState extends State<HotelPage> {
  ...
  List<firebase_storage.Reference> hotelFiles = [];
  ...
  Future<void> listarDocumentos() async {
    final ref = firebase_storage.FirebaseStorage.instance.ref().child('files/${auth.currentUser!.uid}/hotel');
    hotelFiles = (await ref.listAll()).items;
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      floatingActionButton: FloatingActionButton.extended(
        onPressed: () async { await Navigator.of(context).pushNamed('/cad_hotel'); setState(() => _futureListar = listarDocumentos()); },
      ...
      body: FutureBuilder<void>(
        ...
        builder: (context, snapshot) {
          ...
          if (hotelFiles.isEmpty) return EmptyStateWidget(icon: Icons.hotel, message: 'Nenhum documento de hotel\nencontrado.', actionLabel: 'Adicionar agora', onAction: () => Navigator.of(context).pushNamed('/cad_hotel').then((_) => setState(() => _futureListar = listarDocumentos())));
